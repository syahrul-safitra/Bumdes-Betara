                ...
                @foreach ($rentals as $d)
                    <tr class="{{ $loop->even ? "row-even" : "" }}">
                        ...
                        <td class="text-center">
                            {{ $d->tanggal_dikembalikan ? date("d/m/y", strtotime($d->tanggal_dikembalikan)) : "N/A" }}
                        </td>
                        <td class="text-center">
                            <small>{{ $d->status_rental }}</small>
                        </td>
                        @php
                            $sumTotal += $d->total_sewa;
                            $sumDenda += $d->total_denda;
                        @endphp
                        <td class="text-right">{{ number_format($d->total_sewa, 0, ",", ".") }}</td>
                        <td class="text-right">{{ number_format($d->total_denda, 0, ",", ".") }}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr style="background-color: #eee;">
                    <td colspan="6" class="text-right font-bold" style="padding: 10px;">TOTAL KESELURUHAN :</td>
                ...
                <tr style="background-color: #15803d; color: white;">
                    <td colspan="6" class="text-right font-bold" style="padding: 10px;">GRAND TOTAL (SEWA + DENDA) :
                    </td>
